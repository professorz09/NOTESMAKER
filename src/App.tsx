...
const App: React.FC = () => {
    ...
    const content = `
      ...
      <head>
          ...
          <style>
              ...
              table { width: 100%; border-collapse: collapse; margin: 12px 0; border: 2px solid #000 !important; page-break-inside: avoid; font-size: 0.9em; }
  ...
  // --- RENDER ---
  return (
    <div className="flex h-screen w-full bg-slate-50 text-slate-900 font-sans overflow-hidden dot-pattern">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative transition-all duration-300">
         ...
         <div className="flex-1 overflow-auto pt-20 md:pt-32 pb-12 px-0 sm:px-4 md:px-8 relative scrollbar-thin scrollbar-track-transparent">
             {status !== GenerationStatus.IDLE && (
                <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-slate-50/60 backdrop-blur-sm">
                    <div className="bg-white p-8 rounded-3xl shadow-2xl flex flex-col items-center border border-slate-100">
                        <div className="w-16 h-16 border-4 border-blue-600 border-t-transparent rounded-full animate-spin mb-6"></div>
                        <h3 className="text-xl font-bold text-slate-900 mb-2">Creating Content</h3>
                        <p className="text-slate-500 animate-pulse">Analyzing topic • Structuring • Writing...</p>
                    </div>
                </div>
             )}

             <div className={`w-full max-w-[210mm] mx-auto transition-all duration-700 ease-out ${!generatedHtml && status === GenerationStatus.IDLE ? 'opacity-100' : 'opacity-100'}`}>
                 <div 
                    className={`editor-container a4-page editor-content bg-white min-h-[297mm] transition-all duration-300 rounded-sm shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-slate-200/50 ${isEditing ? 'ring-4 ring-blue-500/20 shadow-blue-500/10' : ''}`}
                    style={{ fontSize: `${fontSize}pt` }} 
                 >
                    {!generatedHtml && status === GenerationStatus.IDLE ? (
                        <div className="h-[250mm] flex flex-col items-center justify-center text-center p-6 sm:p-12 border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/50">
                            <div className="w-20 h-20 sm:w-24 sm:h-24 bg-white rounded-2xl shadow-xl shadow-blue-900/5 ring-1 ring-slate-100 flex items-center justify-center mb-6 sm:mb-8 transform hover:scale-105 transition-transform duration-500 rotate-3">
                                <BookOpen className="w-8 h-8 sm:w-10 sm:h-10 text-blue-600 -rotate-3" />
                            </div>
                            <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-800 mb-3 sm:mb-4 tracking-tight">Your Empty Canvas</h2>
                            <p className="text-base sm:text-lg text-slate-500 max-w-md mb-6 sm:mb-8 leading-relaxed px-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 w-full max-w-md opacity-80 px-4">
                                <div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center transition-transform hover:-translate-y-1">
