      ...
      <div className={`rounded-xl border bg-white dark:bg-slate-800/80 px-3 py-2 transition-all ${meta.ring} ${node.status === 'active' ? 'scale-[1.01]' : ''}`}>
        ...
        {node.status === 'error' && (
          <div className="flex items-center gap-2 mt-2">
            <button
              onClick={onRetry}
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-bold text-white bg-indigo-600 hover:bg-indigo-700 active:scale-95 transition-all"
            >
              <RotateCw className="w-3 h-3" /> Retry
            </button>
            <button
              onClick={onSkip}
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[11px] font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 active:scale-95 transition-all"
            >
...
export const MindmapOverlay: React.FC<MindmapOverlayProps> = ({ mindmap, onRetry, onSkip }) => {
  ...
  return (
    <div className="absolute inset-0 z-20 flex items-start justify-center overflow-y-auto p-3 sm:p-6 pt-16 sm:pt-20 bg-slate-50/85 dark:bg-slate-950/85 backdrop-blur-sm">
