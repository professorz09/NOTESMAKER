...
// Phase 2 (per-section expansion) lives in chunkSectionExpansion.ts — one
// call per SECTION (with sub-point batching) instead of one call per chunk,
// so a heading with many sub-points never gets compressed into a summary.
